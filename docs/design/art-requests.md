...
**정직하게 덧붙이면, 이 좁힌 기준을 진짜 키잉 파일로 검증하지는 못했다.** 기준이 되던
`icon-key.png`는 지금 저장소에 있는 것이 이미 다시 뽑은 깨끗한 파일이라(순수 마젠타 0개)
"걸려야 할 것이 걸리는지"를 확인할 대조군이 남아 있지 않다. 옛 키잉 파일을 git 이력에서
꺼내 한 번 돌려 보면 확실해진다.
