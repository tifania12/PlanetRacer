...
- **(2026-09-20 16시 주말 세션) P-14 ②(종 ID 목록) 설계를 끝냈다** — 위 P-14 항목 참고,
  `Core/PetSpecies.cs` 신규. **다음 후보**: (1) 이 종 목록을 실제 뽑기에 연결하는 일
  (`PetGachaController`/`SaveData.PetGachaSave`를 종 단위로 고치는 구조 변경, 위 P-14 항목의
  "아직 안 한 것" 참고) — Core.Tests만으로 검증 가능해 다음 코드 세션도 바로 집을 수 있다.
  (2) B/A/S 등급 부품(루비 행성용 등) 설계로 P-07 레시피를 채우는 것. (3) P-16 티타늄 상자
  드롭률은 여전히 W2 또는 Tifania 결정 대기. (4) Unity 세션이 있으면 U-08(+B-01) 우선,
  그다음 P-06/P-09.
