...
- [ ] P-17 펫 아트 112종. **7등급 10종이 1순위** — 뽑기 화면에 제일 크게 나온다.
      ...
      **1~4등급 색 변종(54종)을 실제로 어느 계열·행성 조합으로 채울지는 아직 코딩 세션이
      혼자 정할 일이 아니다 — `docs/decisions.md` T-12 결정 대기(2026-09-19 발견).**
      6·7등급(신화·초월 48종)의 종 이름·고유 효과 결정 대기와 같은 성격의 블로커다.

## 다음에 할 만한 것 (2026-09-19 11:0x 세션 — 코드로 짤 항목이 계속 바닥난다, 09-18 08:05 목록 갱신)

T-11(컷신 투명 검사)은 09-18에 이미 A안으로 정해져 커밋됐고, W-05(기준점 스크린샷 자동화)도
2026-09-16 Unity 세션에서 `PlayModeWindow.SetCustomRenderingResolution`으로 막힌 부분이
풀려서 둘 다 이 목록에서 뺀다. 지금 진짜로 막고 있는 건 **T-10·T-12** 둘뿐이다 — Core.Tests
커버리지·decisions.md 정리는 09-19 06:05·09:04·10:0x·11:0x 네 세션이 연달아 다시 훑었지만
새로 채울 구멍을 못 찾았다.

- **T-10(증폭기 배율 해석)·T-12(1~4등급 색 변종 54종을 어느 계열·행성 조합으로 채울지)**
  둘 다 최우선. T-10이 정해지면 P-03(`Amplifier.cs`)부터 P-05까지 줄줄이 풀리고, T-12가
  A안으로 정해지면 `art-requests.md`에 계열 4 × 행성 6 = 24줄을 바로 채울 수 있다.
- Unity 세션이 있으면: P-09(행성 선택·이동 화면 uGUI) / U-08(옛 UI Toolkit 루트 정리, 일곱
  화면이 uGUI로 다 옮겨진 뒤가 조건) 중 하나. 07:0x 세션이 남긴 구독 만료 연료 처리·
  `UpgradeUgui` 미리보기 결정도 정해지면 바로 붙일 수 있는 상태로 core가 준비돼 있다.
