...
- [ ] E-01 제작·강화 화면(`CraftingUgui`)에도 화폐 이름을 적는다. 지금은 `제작 (15)`라고만 나와서
      정제 광물이 0일 때 왜 못 누르는지 화면만 보고는 알 수가 없다. 업그레이드 화면은
      2026-09-17에 `제작 (정제 15)` 꼴로 고쳤으니 같은 방식으로.
- [ ] E-02 화폐가 모자라 못 누르는 버튼에 "정제 광물이 모자란다 — 제련소를 올리면 쌓인다" 같은
      한 줄 안내. 지금은 회색 버튼만 보여서 막다른 길인지 그냥 덜 모은 건지 구분이 안 된다.
...
- [ ] P-01 화물칸·엔진 상한을 10 → 30으로 올린다. `UpgradeCost.CargoMaxLevel`·`EngineMaxLevel`.
      올리고 나서 **비용/생산 비율이 1.04~1.06을 유지하는지** 계산으로 확인할 것
      (화물칸은 생산이 선형이라 그대로 두면 비율이 나빠진다 — idle-research.md 1절).
      Core.Tests에 "상한까지 올려도 구매 간격이 N배를 안 넘는다" 테스트를 넣는다
- [ ] P-02 `YieldPerVein`의 10레벨 티어 점프(×1.5)를 늘린다. 지금 3단계뿐이라
      상한을 올리면 중간이 밋밋해진다. AdVenture Capitalist가 25·50·100·200에 ×2를 붙이는 것과 같은 장치
...
- [ ] P-12 `Core/PetGachaTable.cs` — 등급별 확률(일반 70 / 고급 23 / 에픽 6 / 전설 1),
      천장 두 개(10연차 에픽 이상 확정, 80뽑 전설 확정), 조각 환산(5→4→3). 순수 함수, seed 재현
- [ ] P-13 `Core/PetCollection.cs` — 도감 보너스(가진 전부) + 장착 보너스(한 마리).
      **중복이 버려지면 안 된다** — 조각으로 바뀐다
- [ ] P-14 확률 공개 화면. **`PetGachaTable`을 그대로 읽어서 표를 그린다. 사람이 옮겨 적지 않는다.**
      캡슐형/합성형을 나눠서 표시한다(중복→상위 등급은 합성형이라 별도 대상).
      Core.Tests가 "표시값 == 표 값"을 검사한다
- [ ] P-15 펫 아트. `docs/design/art-requests.md`에 프롬프트를 올린다 —
      채굴차에 올라타는 작은 우주 생물, 행성 보석 색, 작아도 실루엣이 읽힐 것
