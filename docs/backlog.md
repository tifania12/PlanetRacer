...
- [ ] W-12 (2026-09-18 07시 배선 세션 발견) **웹에서 켜질 때 URP가 렌더 에러를 108번 토한다.**
      `dev` 주소로 게임을 열면 로딩 직후 `Render Graph Execution error` +
      `ArgumentException: RenderTextureDesc width must be greater than zero (desc.width)`가
      **108번** 몰아서 찍힌다. 그다음부터는 한 번도 안 늘어난다(8초 뒤에도 108 그대로) —
      즉 한 번 터지고 끝나는 시작 구간 문제다. 화면은 정상으로 보이고 게임도 잘 돌아서
      **눈으로는 안 보인다.** 콘솔을 봐야 보인다.
      **원인 추정**: 캔버스 폭이 0인 순간에 URP가 렌더 텍스처를 만들려고 한 것. 확인 당시
      캔버스는 1024×768로 정상이었으니, 창이 잡히기 전(또는 크기가 바뀌는 중)의 한때로 보인다.
      **판단 보류 이유**: 이번 확인은 자동화 브라우저 창에서 한 것이라 그 창이 뜨면서 크기가
      0에서 시작한 탓일 수 있다 — 그러면 Tifania 폰·PC에서는 안 나는 것이 된다. 다만
      **폰에서 화면을 돌리거나 주소창이 접히는 것도 같은 크기 변경**이라, 손으로 한 번
      브라우저 창을 끌어서 좁혔다 넓혀 보면 재현되는지 바로 알 수 있다. 재현되면 폰에서
      돌릴 때마다 108번씩 도는 셈이니 가볍지 않다.
      **다음 세션 몫**: (1) 창을 실제로 리사이즈해 재현 여부 확정 (2) 재현되면 카메라·URP
      Render Graph 쪽에서 폭 0을 걸러 내는 자리 찾기 (3) 안 되면 이 항목을 자동화 창 문제로
      닫기. 어느 쪽이든 결론을 이 항목에 적을 것
...
- [ ] P-03 `Core/Amplifier.cs` — 등급(일반/고급/에픽/전설) → 증폭률. 순수 함수.
      ...
      **Tifania에게 물어볼 것이 하나 남아 있다** — "0.01~10배"를 증폭률 범위로 읽을지
      최종 배율 범위로 읽을지. A안(증폭률)으로 문서를 썼는데 확인이 필요하다
...
- [ ] P-10 코스 데이터 절차적 생성. `Course`가 이미 길이·랩·평지/험지/부스트 비율을 갖고 있으니
      시드에서 그 네 값을 뽑는 순수 함수부터. 행성마다 수십 개가 목표
...
- [ ] P-17 펫 아트 112종. **7등급 10종이 1순위** — 뽑기 화면에 제일 크게 나온다.
      계열 넷(바퀴족·날개족·광석족·짐꾼족)으로 묶고 등급은 같은 생물이 자란 모습으로 그린다.
      1~4등급은 행성 색 변종으로 채워서 실제 신규 작화를 74장으로 줄인다(pet-gacha.md 7절).
      프롬프트 고정문이 그 절에 있다. `docs/design/art-requests.md`에 7절의 순서대로 올린다

## 다음에 할 만한 것 (2026-09-18 08:05 세션 — 코드로 짤 항목이 바닥났다)

Core.Tests의 `throw new` 방어 코드 26개 파일을 전부 대조했지만 새 구멍이 없었고, backlog
위쪽은 전부 Unity 에디터가 있는 세션 몫이거나 T-10(증폭기)·T-11(컷신 투명 검사) 결정
대기다. 지어내지 않고 후보만 남긴다 — Tifania가 보고 고르면 된다.

- T-10·T-11 둘 중 하나라도 정해지면 그게 제일 값어치 있다. T-10이 정해지면 P-03(`Amplifier.cs`)
  부터 P-05까지 줄줄이 풀리고, T-11이 정해지면 컷신 7장이 바로 커밋된다.
- Unity 세션이 있으면: U-08(옛 UI Toolkit 루트 정리, 관문 풀림) / W-12(URP 렌더 에러 108번
  재현) / D06-N(광맥 위치 기반 이동 로직) / P-09(행성 선택·이동 화면 uGUI) 중 하나.
