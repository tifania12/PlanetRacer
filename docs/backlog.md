...
- [ ] M-01 화물칸 상한을 접속 중에도 적용. 코어 `MiningSimulator`에 온라인 틱용 상한 클램프 추가. 상한은 절대량이 아니라 "그 행성 기준 N시간치 산출"로 계산(쿼츠·루비 4h, 사파이어·아쿠아마린 5h, 주사·라피스 6h). 테스트: 상한 도달 후 추가 시간에도 원석이 안 늘어나는지
- [ ] M-02 정제 광물은 화물칸을 차지하지 않게 분리. 원석만 상한에 걸린다. 자동 제련소 레벨이 원석→정제 변환 속도를 정하고, 이게 실질 상한을 올리는 무료 해법이 된다. 테스트: 제련소 레벨이 오르면 같은 시간에 더 오래 캐지는지
- [ ] M-03 첫 상한 도달까지 최소 90분 보장. 봇 시뮬레이션으로 실제 시각을 측정하고, 90분보다 이르면 쿼츠 상한을 올린다. 이 줄이 깨지면 초반 이탈이 난다 — 수치보다 중요한 관문
- [ ] M-04 상한 도달 UI. 첫 화면은 "정제로 돌리시겠어요?"(무료 해법)를 먼저 보여 주고, 상점은 그다음. "멈췄습니다"로 시작하지 않는다
- [ ] M-05 화물칸 80% 푸시 알림. 재접속을 만드는 장치라 상품 하나보다 매출 기여가 크다. 로컬 알림으로 먼저 구현(서버 푸시는 P3)
- [ ] M-06 코어 `Entitlements.cs` — 구매·구독 상태를 한 곳에서 계산. 구독과 영구 구매가 겹치면 "더 큰 값 적용, 중복 차감 없음". UI는 계산 결과만 읽는다. 테스트: 중복 적용 안 되는지
