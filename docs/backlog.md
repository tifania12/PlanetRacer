...
일과 (2026-09-23 갱신 — 자세한 것은 CLAUDE.md, 이 절이 실제 스케줄과 어긋나 있던 것을 바로잡음)
- 평일 18:00~08:00 매시간(하루 15번) — 한 번에 1~2항목. 옛 "23:00/03:00에 2~3항목씩 크게"는
  더 이상 맞지 않는다 — 지금은 평일 밤도 주말처럼 매시간 돈다.
- 평일 06:30 — 작게. 검증·배포 확인·아침 가이드만. 새 기능 금지
- 평일 08:00~18:00 — 안 돎. Tifania가 회사에서 쓰는 시간, 그 몫을 남긴다
- 주말(+공휴일) 매시간(하루 24번) — 한 번에 1~2항목. 예전엔 하루 네 번(09/14/19/23시)에
  3~4항목씩 크게 갔지만, 지금은 매시간 도니 그렇게 하면 세션끼리 겹친다.
...
- [ ] **A-17 펫 뽑기 화면 + 도감.** 펫 그림 123장이 한꺼번에 살아난다. 수익 화면이기도 하다.
      ...
      **조각 합성(`PetFusion`) 실행 화면 — 코어 쪽 끝남(2026-09-22 00시 코딩 세션).**
      `Core/PetFusionController.cs`(신규) — `FuseSameGrade`(조각 3개당 같은 등급 다른 펫 1마리,
      종 선택은 뽑기와 같은 규칙이라 중복이면 조각으로 자동 환급)와 `FusePromotion`(조각을
      ...
      **`GemRacer/29` 배선 끝남(2026-09-22 03시 Unity 배선 세션).** 메뉴 실행 → 콘솔 에러 0 →
      ...
      확정(Tifania 판단)뿐이다.**
      **(2026-09-22 06시 세션) 판단 대기 정리해서 올림 — `docs/decisions.md` 맨 아래 절 참고.**
      셋 중 진짜 걸린 건 **일반 뽑기** 하나뿐이었다 — 고급/특수는 `pet-gacha.md` 4절에 실물결제
...
- [x] U-11 (2026-09-26 19시 Unity 배선 세션 발견 → 같은 날 21시 Unity 배선 세션에서 고침)
      **(발견 당시 기록 — 배포된 dev 주소를 가로로 열어 보다가 잡았다)**
      ...
      **→ 이 안(`ResponsiveGridCell`, 문턱값 700)은 아래 것으로 합쳐졌다.** 같은 시각 Unity
      배선 세션이 같은 항목을 따로 고치고 있었고, push 때 충돌로 드러났다. 에디터에서 컴파일·
      세 기준점 실측·눈 확인까지 끝나고 씬에도 이미 붙어 있던 쪽(아래)을 남기고
      `ResponsiveGridCell.cs`는 지웠다(씬에서 쓰인 적 없음).
      **고친 방법(2026-09-26 21시).** 화면마다 숫자를 고치지 않고 공통 컴포넌트를 하나 만들었다 —
      `Assets/Scripts/UI/ResponsiveGridCells.cs`. 부모 폭에서 들어갈 칸 수를 먼저 정하고
      (minCellWidth 330 기준, 최대 2열) 남은 폭을 그 칸 수로 정확히 나눠 `cellSize.x`에 넣는다.
      `Bootstrap*Ugui` 여섯 개(제작·레이스 출전·공구 상자·상점·업그레이드·시즌 패스)가 grid를
      만든 자리에서 이 컴포넌트를 같이 붙이게 했고, 이미 배선돼 있던 `MainGame.unity`의 여섯
      `row-list`에도 `execute_code`로 직접 붙였다(`GemRacer/7`은 누르지 않았다 — CLAUDE.md 함정 절).
      **세 기준점 실측**: 세로 540×960 → 폭 500 / 1열 / 칸 500, 가로 960×540 → 폭 920 / 2열 /
      칸 454, 태블릿 1280×800 → 2열. 상점 화면으로 세 장 다 눈으로 확인했고 한글도 멀쩡하다.
      **세로 모습이 조금 바뀐다** — 전에는 칸이 400으로 고정이라 오른쪽에 100쯤 빈 띠가 있었는데
      이제 폭을 꽉 채운다. 정보는 그대로이고 여백만 없어진 것이라 그대로 뒀다.
      **안 건드린 것**: 펫 도감(`dex-grid-*`)·아트 확인 화면·펫 뽑기 결과는 칸이 150/72라 이미
      여러 열로 깔린다. 다만 가로에서도 세로와 같은 3열 고정이라 오른쪽이 남는데, 이건 칸 크기가
      아니라 `FixedColumnCount` 고정값 문제라 성격이 다르다 — 다음 Unity 세션이 같은 컴포넌트에
      최대 열 수만 올려 붙이면 될 것 같다(급하지 않다).
...
- [ ] D18-N (9/29 화) 리텐션 훅: 화물칸이 다 찼을 때 로컬 알림(Android/iOS Mobile Notifications 패키지), 하루 첫 접속 보상.
  **(2026-09-25 03시 Unity 배선 세션) "하루 첫 접속 보상" 축은 화면까지 다 붙었다.**
  `GemRacer/31` 실행 → `UI Canvas/Overlays/DailyLoginReward` 생성 → 씬 저장까지 끝냈다.
  컴파일 에러 0, Play 13초 예외 0, 실제 값 **원석 8.0 · 연속 접속 2일째 · 정제 광물 15.0**
  (23시 세션의 5원석·1일에서 KST 자정을 넘기며 보상표대로 올라간 것까지 확인).
  "확인" 버튼도 눌러 `AcknowledgeDailyRewards()` → 다음 프레임 자동 닫힘 확인.
  `MainHudUgui` 칸 연결은 없다 — `OfflineRewardUgui`처럼 HUD 버튼 없이 스스로 뜨는 구조다.
  ...
  **→ (2026-09-26 22시 주말 세션) 선택지를 `docs/decisions.md`에 A/B/C안으로 올렸다** —
  "지금 판단 대기 중인 것" 목차와 "D18-N 화물칸 오버플로" 절 신규. 조사해 보니 이 결정은
  M-13(오프라인 상한 연장 상품)과 사실상 같은 질문(정제 광물에 오프라인 상한을 걸 것인가)이라
  M-13을 먼저 정하면 이 항목도 대부분 같이 풀린다 — decisions.md에 그렇게 적어 뒀다.
...
- [ ] P-07 행성별 광물 종류. 상위 행성 광물이 상위 부품 제작에 쓰이게 해서 되돌아갈 이유를 만든다
      ...
      **(2026-09-27 04시 주말 세션 정정)** "창고 연결이 Unity 세션 몫"이던 이유(P-09 UI 흐름이
      먼저 있어야 정할 수 있다)는 P-09가 2026-09-25에 끝나면서 이미 사라졌다 — 이제 남은 건
      **Tifania 결정 대기**(정제되는 즉시 vs 행성을 떠날 때 vs 레이스 우승 시점)뿐이다.
      `docs/decisions.md` "정제 광물을 행성별 창고로 언제 옮길지" 절에 A/B/C안 올림. 정해지면
      코딩 세션이 `MiningController.cs` 한두 줄로 바로 끝낼 수 있다(A안이면 239줄 옆에
      `PlanetMineralBank.Add` 한 줄).
...
- [ ] P-16 **초월의 인장** — 티타늄 상자 희귀 드롭 / 행성 클리어 / 시즌 패스 / 유료.
      ...
      **(2026-09-26 23시 세션) "고정 수량이라 결정이 필요 없었다"고 했던 세 경로 중 시즌 패스
      축은 사실 그렇지 않았다** — `DefaultData.SeasonPassTiers()`의 열 레벨이 전부 FreeReward가
      이미 채워져 있어서, 인장을 끼워 넣으려면 기존 보상 중 하나를 밀어내거나(A안)
      `SeasonPassTier` 구조를 바꾸거나(B안) 레벨을 더 늘려야(C안) 한다 — 셋 다
      `docs/decisions.md` "시즌 패스 무료 트랙 인장" 절에 옵션으로 올려 Tifania 판단 대기로
      돌렸다. 티타늄 상자 축(위 W2 대기)과 별개로 이 축도 이제 판단 대기다.
...
- [ ] P-19 (2026-09-27 03시 주말 세션 발견, CLAUDE.md 규칙 넷 — code-only 신규 항목이 세
      세션 연속 바닥나서 설계 문서를 다시 훑다가 찾음) **펫을 아무리 모으고 장착해도 게임
      플레이에 아무 효과가 없다.** `pet-gacha.md` 2절 표(도감 보너스 +1~+35%, 장착 효과
      +8~+55%)와 8절("MiningSimulator·레이스 스탯에 펫 보너스 반영")이 둘 다 요구하고 있고,
      코어 함수 `PetCollection.CollectionBonus(int[])`/`PetCollection.EquipBonus(PetGrade)`도
      이미 만들어져 테스트까지 통과하는데(A-17, 2026-09-19~21 세션들), **이 두 함수를 부르는
      곳이 코드 전체에 0군데다**(`grep -rn "PetCollection\." Assets` 0건 — `MiningSimulator`·
      `RaceSimulator`·`MiningController` 어디에도 없음). A-17이 "코드 쪽은 전부 끝났다"고
      닫은 뒤에도 이 자리는 한 번도 안 걸렸다 — 뽑기·합성·도감·장착 UI는 전부 동작하지만
      **장착해도 아무것도 안 변한다**는 뜻이라, 지금 상태로 나가면 수익 화면(A-17)의 핵심인
      "장착하면 강해진다"는 약속이 거짓이 된다.
      **배선을 미룬 이유(설계 판단 필요)**: 이 보너스 %가 정확히 **어느 스탯에** 붙는지가
      `pet-gacha.md` 어디에도 안 적혀 있다. 후보가 여럿이라 임의로 하나를 고르면 나중에
      뒤집힐 위험이 크다 — 채굴 산출(`MiningSimulator.GemsPerHour`/원석 산출)? 정제 속도?
      화물칸? 레이스 속도(`RaceSimulator`)? 여러 스탯에 나눠 붙는 구조(위 "7등급 10마리는
      서로 다른 축을 하나씩 맡는다"는 문장이 힌트일 수 있음)? `Entitlements.Effective`처럼
...
- **(2026-09-21 02시 주말 세션) 위 (1)번 — A/S 등급 쿼츠 부품을 설계해 P-07 레시피를 채웠다.**
  ...
  **(2026-09-22 18시 야간 세션) "P4 봇 시뮬레이션에서 재조정 여지 있음" 각주를 처음으로
  실제로 돌렸다.** `Core.Tests/BalanceSimParts.cs` 신규(`dotnet run -- sim-parts`, L-05
  `BalanceSim.cs`와 같은 이유로 평소 테스트엔 안 낌) — 쿼츠 코스 3개에서 맨몸 대비 C/B/A/S
  5부품 완착 시 `RaceSimulator.LapTime`을 비교했다. **결과가 가설과 반대였다** — 비용은
  매 등급 ×4로 일정한데, 시간 단축 폭은 등급이 오를수록 22.4%→30.7%→37.8%로 **점점 커진다**
  (수확 체감이 아니라 체증). `LapTime`이 "상수항 + 스탯 비례항"의 역수 구조라 스탯이 커질수록
  같은 배수 증가의 효과가 커지는 게 원인. 값은 안 고쳤다 — 어느 쪽을 조정할지(스탯 배수 축소 /
...
- [x] B-01 **(2026-09-24 19시 Unity 배선 세션 — 고쳐졌고 dev 주소에서 확인했다)**
      웹 빌드가 뜨는 순간 콘솔에 `Render Graph Execution error` + `ArgumentException:
      ...
      **다만 아직 확인된 게 아니다** — 이 세션은 에디터 Play에서만 봤고(예외 0), B-01은
      ...
      → **같은 세션이 그 확인까지 했다. 0줄이다.** 빌드 463(`a5d25ce`) 배포 후 dev 주소를
      IndexedDB(`/idbfs`·`UnityCache`) 비우고 새 탭에서 열었다 — 로드 중 콘솔 126줄에
      `Render Graph Execution error` **0건**, 로드 뒤 20초 정상 상태에서도 **0건**.
      화면도 정상(튜토리얼 1/4, 한글 정상, 버튼 여덟 개, 행성·채굴차 렌더 정상).
      **2026-09-20 08시의 진단이 맞았다** — UIDocument 열 개가 원인이었다.

      ⚠️ **다음에 이걸 확인할 사람이 꼭 읽을 것 — 하마터면 "안 고쳐졌다"고 적을 뻔했다.**
      처음엔 에러가 **222줄**로 오히려 늘어난 것처럼 보였다. 원인은 빌드가 아니라 **보는 쪽**이었다:
      Claude 브라우저 창(background pane)에서 열면 캔버스가 **1x1**로 잡히는데, B-01은 애초에
      "폭 0짜리 캔버스에서 RT를 만들려다 나는 에러"라 **관찰 조건 자체가 같은 에러를 만들어 낸다.**
      즉 이 버그는 자기를 확인하려는 도구에 그대로 재현되는 종류다. `canvas.width`를 먼저 찍어
      보고 1x1이면 그 측정은 버린다 — 뷰포트를 375x812 같은 실제 크기로 맞추고 **다시 로드한 뒤**
      세야 한다(크기만 바꾸고 새로고침을 안 하면 이미 난 에러가 버퍼에 남아 있어 또 속는다).
...
## 다음에 할 만한 것 (2026-09-26 16시 주말 세션이 남김 — 직접 시작하지 않음, Tifania가 고를 것)

- monetization.md 2-7 **스킨 직접 구매(₩3,300~8,800)**. 지금은 시즌 패스 유료 트랙 보상으로만
  `Cosmetic`(문자열 id, `SaveData.OwnedCosmeticIds`)이 생긴다 — 상점에서 직접 사는 경로,
  실제 스킨 카탈로그(어떤 스킨이 몇 종 있는지)는 아직 아예 없다. monetization.md 자신도 "출시
  후 추가 매출"로 분류해 둔 항목이라 지금 급하지 않고, 그림(A-2x류)이 없어도 문자열 카탈로그
  정도는 code-only로 시작할 수 있지만 "몇 종을 얼마에 팔지"는 설계 판단이 필요해서 이번 세션은
  건드리지 않았다.
