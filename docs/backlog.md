...
- [ ] **T-14 WebGL 빌드가 네 번 연속 죽었다 — dev 주소가 09-20 낮부터 멈춰 있다 (19시 Unity 세션 확인).**
      **다음 세션이 이것부터 본다.** CLAUDE.md "빌드 실패는 다음 세션이 최우선으로 고친다".
      | run | 커밋 | 결과 |
      |---|---|---|
      | #339 | `e516b23` (P-14) | **성공 — dev 주소는 아직 이 화면이다** |
      | #340 | `95392a3` | 실패 exit **255** |
      | #341 | `5856dab` (A-21) | 실패 exit **255** |
      | #342 | `46f5b0d` (A-16) | 실패 exit **139** |
      **컴파일 에러가 아니다.** 주석(annotation)에 `error CS`가 한 줄도 없고 경고뿐이다.
      죽는 자리는 `game-ci/unity-builder@v4` 단계이고, 그 뒤 배포 단계는 전부 skipped다.
      exit 139는 SIGSEGV(세그폴트)라 **유니티가 빌드 도중 죽은 것**이다 — T-13 때도
      "세 번 연속 exit 139"가 있었고 그때는 메모리였다. 워크플로에 이미
      「스왑 추가 (IL2CPP 메모리 부족 대비)」 단계가 있는데도 죽는다.
      **#340이 문서만 고친 커밋이라는 게 실마리다** — 프로젝트 코드 탓이 아닐 가능성이 크다.
      의심 순서: (1) 러너 메모리 — `Resources`에 아직 PNG 155장이 들어가고 IL2CPP가 같이
      돈다, 스왑을 더 키워 본다. (2) `Library` 캐시(`actions/cache@v4`)가 #339 이후 오염됐을
      가능성 — 캐시 키를 한 번 갈아 본다. (3) 유니티 라이선스/이미지 버전 변동.
      확인은 **목록 아이콘 말고 API로** 한다(CLAUDE.md 2026-09-16 절).
      이것 때문에 **오늘 한 일(A-21 · A-16)이 웹에서 하나도 안 보인다.**
