...
- [ ] **A-21 펫 폴더에 남은 옛 파일 26장을 정리한다.** `Art/Pets/` 바로 아래에
      `pet-t1-carrier.png` 같은 옛 이름 규칙 파일이 26장 있고, 등급 폴더의 123장과 겹친다.
      빌드 용량을 그대로 먹고 `?art=1`에도 두 벌로 나온다.
      **파일을 지우는 일이라 Tifania 확인 전에는 손대지 않는다.**
