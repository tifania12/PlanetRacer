...
## 다음에 할 만한 것 (2026-09-20 03시 주말 세션 갱신 — PlanetMineralRecipe 버그 수정)
...
- **코드만으로 되는 다음 후보는 마땅한 게 바로 안 보인다** — P-07의 남은 절반(레시피 값)은
  부품 데이터가 없어서 못 채우고, P-08(행성 클리어 조건)은 Tifania 결정 대기다. 다음 세션은
  아래 "바닥났을 때" 후보(Core.Tests 경계값 보강, 밸런스 시뮬레이션, 문서 정리)에서 고르거나,
  B/A/S 등급 부품(루비 행성 부품 등) 자체를 새로 설계해 DefaultData에 추가하는 걸 검토할 만하다
  — 그러면 P-07 레시피도 같이 채울 수 있다.
