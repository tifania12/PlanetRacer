...
- [?] **HUD 액션 줄이 9칸이 되면서 긴 라벨이 잘린다.** (2026-09-25 07시 Unity 배선 세션에서
  ...
  **다만 8칸이던 때에도 이미 잘리고 있었다**(btn-planet을 껐다 켜며 A/B로 확인) — 이번
  ...
  — **2026-09-26 19시 갱신(실제로 눌러 보고 잰 값)**: `GemRacer/34`를 눌러 10칸이 됐다.
  칸 너비는 **9칸 50px → 10칸 44.4px**(action-row 전체 516px). 라벨이 실제로 잘리는 버튼이
  **둘에서 셋으로 늘었다**:
  - `btn-mine` "업그레이드" — 필요 86.4px / 있는 자리 44.4px, 최소 크기 14pt에서 더는 못 줄어듦
  - `btn-race` "레이스" — 필요 51.9px / 44.4px, 17.1pt까지 줄었는데도 모자람 **(이번에 새로 잘림)**
  - `btn-seasonpass` "시즌패스" — 필요 69.1px / 44.4px, 12.8pt까지 줄었는데도 모자람 (새 버튼)
  나머지 일곱 개는 두 글자라 20pt 그대로 들어간다. **앞 기록이 걱정하던 "짧은 라벨까지
  위험해진다"가 실제로 일어났다** — 세 글자짜리 "레이스"가 이번에 넘어갔다. 다음에 버튼이
  하나만 더 붙으면 40.4px가 되어 두 글자 라벨도 20pt로는 안 들어간다(34.6px 필요,
  여백까지 치면 아슬아슬).
  참고로 `btn-seasonpass`만 `fontSizeMin`이 12pt다(나머지 아홉 개는 14pt) — 더 줄어들 수
  있어서 다른 버튼보다 글자가 작다. 위 (1)~(3) 중 무엇으로 갈지 정해지면 이 값도 같이
  맞추는 게 좋다. **여전히 Tifania 결정 대기** — 취향 문제라 세션이 임의로 정하지 않는다.
