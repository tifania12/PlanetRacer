...
Tifania가 ChatGPT Plus를 쓰고 **이미지 생성은 거기서 한다.** 자동 세션은 GPT에 접근할 수 없다
(커넥터가 없고, 계정 로그인을 대신 할 수도 없다).

그러니 이미지가 필요해지면 **직접 만들려 하지 말고 `docs/design/art-requests.md`에 프롬프트를 적는다.**
영문 프롬프트로, GPT에 그대로 붙여 넣을 수 있게. Tifania가 뽑아서 `Assets/Art/`에 넣어 준다.
그리고 daily 파일에 "이미지 요청 N건 올림" 한 줄을 남겨 아침에 보이게 한다.
