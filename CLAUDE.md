...
회사 일이 없는 날이라 더 많이 돈다. 09:00 / 14:00 / 19:00 / 23:00 네 번, 전부 크게.
평일과 다른 점은 **하루 안에 되돌아볼 수 있다**는 것이다. 그래서 주말에는 평일에 못 하는 종류의 일을 붙인다.

- 09:00 — 평일에 밀린 `- [?]` 피드백과 막힌 항목부터. 판단이 필요한 것을 정리해 `docs/decisions.md`에 선택지로 올린다.
- 14:00 — backlog 3~4항목. 큰 구조 변경이나 리팩터링은 여기서 한다.
- 19:00 — 14:00 결과 위에 이어서 3~4항목.
- 23:00 — 밸런스 시뮬레이션, 테스트 보강, 문서 정리처럼 오래 도는 일. 다음 날 아침 가이드 작성.
